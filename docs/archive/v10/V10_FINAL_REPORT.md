...
The manuscript now centers the directed update `pi -> pi'`, states all six propositions, defines ISR/CTI/CISR and matched-cell FER, removes internal run/version language from the scientific body, separates PIVOT-VOI from PIVOT-H, and includes an AI disclosure matched to the documented tooling. The current PDF has `7` main pages and `13` total pages; references and appendix are outside the main-text gate.
