...
_implementation_assistant_RE = re.compile(r"implementation_assistant", flags=re.IGNORECASE)
...
def scan_language(root: Path) -> dict[str, Any]:
    ...
    source_path = root / "paper/main.tex"
    body_sources, body = _body_source_closure(source_path)
    pdf_path = root / "paper/pivot_iclr2027_submission.pdf"
    try:
    ...
    release_root = root / "paper/figures/release"
    release_text = ""
    ...
    reviewer_implementation_assistant_hits = sorted(set(_implementation_assistant_RE.findall(reviewer_text)))
    body_source_hits = {
    ...
    payload = {
        ...
        "paper_facing_implementation_assistant_tokens": len(_implementation_assistant_RE.findall(body + pdf_text)),
        "reviewer_artifact_implementation_assistant_tokens": len(_implementation_assistant_RE.findall(reviewer_text)),
        "reviewer_artifact_implementation_assistant_hits": reviewer_implementation_assistant_hits,
        "valid": not _VERSION_RE.search(body)
        and not _VERSION_RE.search(pdf_text)
        and not _implementation_assistant_RE.search(body + pdf_text)
        and not reviewer_implementation_assistant_hits,
    }
    return payload
