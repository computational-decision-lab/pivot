...
def _number_audit(root: Path) -> dict[str, Any]:
    ...
    macro = root / "paper/results_macros.tex"
    if not macro.is_file():
        macro = root / "paper/v10_results_macros.tex"
...
def _pdf_summary(root: Path) -> dict[str, Any]:
    ...
    verification = _json(root / "paper/verification.json", {})
...
- Forbidden implementation-assistant tokens in paper/PDF: `{language['paper_facing_implementation_assistant_tokens']}`
- Forbidden implementation-assistant tokens in reviewer artifacts: `{language.get('reviewer_artifact_implementation_assistant_tokens', 0)}`
