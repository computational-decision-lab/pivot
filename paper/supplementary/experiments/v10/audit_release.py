...
def pre_final_audit(root: Path) -> dict[str, Any]:
    ...
    snapshot_pdf = snapshot / "paper/pivot_iclr2027_submission.pdf"
    snapshot_zip = snapshot / "paper/pivot_iclr2027_supplementary.zip"
    snapshot_manifest = snapshot / "SHA256SUMS"
    old_verification = load_json(snapshot / "paper/verification.json")
    issues = [
        {
