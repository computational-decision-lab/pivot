...
_PRIVATE_TEMP_PATH = re.compile(r"<local-root>[A-Za-z0-9_.-]+(?:/[A-Za-z0-9_.@+-]+)*")
...
def finalize(root: Path) -> dict[str, Any]:
    ...
    _run(["bash", "paper/build.sh"], root, steps)

    # The build regenerates figures/tables, so audit their final bytes again.
    audits = _run_audits(root)
    paper_verification = load_json(root / "paper/verification.json")
    ...
    finalize_report: dict[str, Any] = {
        ...
        "paper": {
            ...
            "sha256": sha256(root / "paper/pivot_iclr2027_submission.pdf"),
        },
