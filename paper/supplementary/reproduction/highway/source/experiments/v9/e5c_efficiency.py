                ...
                for budget in budget_values:
                    for method in config["methods"]:
                        ...
                        for row in candidates:
                            ...
                            queried = str(row["transition_id"]) in selected_ids
                            copy["hf_queried"] = queried
                            if queried:
                                copy["observed_delta"] = float(row["delta_true"])
                            rows.append(copy)
...
def _candidate_groups(rows: list[dict[str, Any]], max_groups: int) -> list[list[dict[str, Any]]]:
    ...
    # Robustness reruns acquisition from the common ex-ante candidate pool.
    # Use the zero-query arm rather than whichever method/budget duplicate was
    # appended last, which may correctly carry an observed HF outcome.
    source = [
        row
        for row in rows
        if row.get("method") == "proxy_only" and int(row.get("queries", -1)) == 0
    ]
    for row in source:
