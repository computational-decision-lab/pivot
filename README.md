...
- V10 release status: the manuscript, six-proposition theory, PIVOT-VOI
  architecture, redesigned main/appendix figures, number/claim/bibliography
  audits, and deterministic supplement are generated by one finalizer. The
  package conclusion is `READY_FOR_SUBMISSION`; OpenReview metadata, the
  matching AI-use form disclosure, and other author-side checks remain outside
  the repository.
...
It rebuilds the pinned architecture and all ten V10 figure bundles, regenerates
the paper and supplement, runs the release audits, and writes
`V10_FINAL_REPORT.md`. It never reruns or tunes confirmatory experiments.
...
The V9 confirmatory package remains the frozen source evidence consumed by V10.
Its historical runner and analysis commands are retained below for provenance;
the V10 finalizer is the supported submission workflow.
