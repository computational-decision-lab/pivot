...
def main():
    out=ROOT/'melting_recovery_20260915/hierarchical_stratified_smoke'
    ...
    for k in ['OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','TF_NUM_INTRAOP_THREADS','TF_NUM_INTEROP_THREADS']:
        env[k]='1'
    states=[]
    for phase,seed,target in [('calibration',39100,out/'calibration/seed_39100'),('test',39101,out/'test_39101')]:
        cmd=[PY,str(ROOT/'melting_hierarchical_e5_stratified.py'),'--phase',phase,'--seed',str(seed),'--output',str(target),
             '--model-path',str(MODEL),'--crossbench-root','/root/autodl-tmp/colin_melting',
             '--k','2','--budgets','0,1,2','--horizon','100','--candidate-train-episodes','1',
             '--response-train-episodes','1','--stratified-replicates','1','--proxy-stratified-replicates','1',
