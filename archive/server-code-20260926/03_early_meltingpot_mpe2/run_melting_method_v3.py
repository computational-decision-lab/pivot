...
def main():
    ...
    p.add_argument('--workers',type=int,default=30);p.add_argument('--max-seconds',type=int,default=14400);a=p.parse_args()
    protocol=json.loads(a.protocol.read_text());assert protocol['status']=='FROZEN' and protocol['test_seeds']==list(range(44000,44030))
    assert 1<=a.workers<=30 and 0<a.max_seconds<=14400
    ...
    def run(seed):
        ...
        cmd=[sys.executable,str(root/'melting_method_native_v3.py'),'--protocol',str(a.protocol.resolve()),'--seed',str(seed),'--output',str((a.output/f'seed_{seed}').resolve()),
