...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--output',type=pathlib.Path,required=True);args=parser.parse_args();out=args.output;out.mkdir(parents=True,exist_ok=False)
    ...
    for task in ['melting_pd','melting_stag']:
        ...
        p={r['seed']:r for r in task_records if r['method']=='pivot_voi' and r['budget']==16};u={r['seed']:r for r in task_records if r['method']=='uniform_paired' and r['budget']==16}
        tables[task]={'n_seeds':6,'methods':methods,'prespecified_primary_difference_pivot_minus_uniform16':ci([p[s]['audit_gain']-u[s]['audit_gain'] for s in range(1300,1306)]),'mechanism':{key:ci([r[key] for r in mechanism]) for key in ['response_effect','candidate_specific_effect','response_own_reward_gain']}}
