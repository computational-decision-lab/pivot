...
for path in (PANEL / 'episodes').glob('*.json'):
    ...
    if row['signature']['key'] in (['training', 0, 0], ['training', 0, 1], ['training', 0, 2]):
...
try:
    for path, original in rows:
        ...
        checks.append({'original_path': str(path.relative_to(BATCH)),
                       ...
                       'match': match, 'original_policy_keys': original['policy_initial_random_keys'], 'replay_policy_keys': replay['policy_initial_random_keys'], 'original_trajectory': original['trajectory_sha256'],
...
(BATCH / 'replay_followup.json').write_text(json.dumps(result, indent=2)+'\n')
