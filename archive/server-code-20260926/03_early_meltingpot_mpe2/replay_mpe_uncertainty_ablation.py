...
def main():
    p=argparse.ArgumentParser();p.add_argument('--base',type=pathlib.Path,required=True);p.add_argument('--output',type=pathlib.Path,required=True);p.add_argument('--repo',type=pathlib.Path,required=True);p.add_argument('--discrepancy-calibration',type=pathlib.Path,required=True);a=p.parse_args()
    ...
    calibration_estimate=read(a.discrepancy_calibration);tau2=float(calibration_estimate['discrepancy_variance_estimate'])
    assert calibration_estimate['not_tuned_on_test'] is True and tau2>=0
    dim=posterior.feature_dim;cov=np.zeros((dim+4,dim+4));cov[:dim,:dim]=posterior.covariance;cov[dim:,dim:]=tau2*np.eye(4)
    posterior=BayesianLinearDeltaPosterior(prior_precision=1.,noise_variance=noise,mean=np.r_[posterior.mean,np.zeros(4)],covariance=cov,n_observations=posterior.n_observations)
    config.update(discrepancy_variance=tau2,discrepancy_calibration_sha256=sha(a.discrepancy_calibration),augmentation='4 independent candidate random effects; zero mean, variance calibrated without test labels; reset each panel',point_predictions_unchanged=True)
    write(out/'protocol.json',config)
    ...
    for seed in config['test_seeds']:
        ...
        arr=np.array(features['rows']);z=np.c_[np.ones(4),(arr[:,1:]-mean)/scale,np.eye(4)]
    ...
    summary={'status':'complete','purpose':config['purpose'],'method_variant':config['method_variant'],'discrepancy_variance':tau2,'source_commit':COMMIT,'n_calibration_seeds':24,'n_panel_seeds':30,'n_candidates':120,'n_decisions':len(records),'decisions_sha256':frozen_hash,'audit_isolated':True,'isolation_check_scope':'within each independent trained candidate panel','source_worktree_unchanged':not subprocess.check_output(['git','-C',str(a.repo),'status','--porcelain','--untracked-files=no'],text=True).strip(),'new_environment_steps':0,'new_api_calls':0,'query_package_definition':'4 historical paired response replicates, 128 evaluation episodes each; paid independently for each method','results':table,'exploratory_paired_differences':differences,'predictor_mse_vs_noisy_audit':{'proxy':float(np.mean((proxy-truth)**2)),'shared_bayesian_correction':float(np.mean((pred-truth)**2))},'limitations':['Previously inspected historical test panel; development evidence only.','MPE-specific features and calibrated noise differ from original controlled worlds.','Upstream generic entry uses one pre-query stop decision and a batch; no sequential posterior updating.','Independent candidate discrepancy is a calibration-based approximation; not an original author configuration or guaranteed calibration.','Negative audit means are noisy estimates, not confirmed harmful upgrades.','Cost excludes shared pretraining and calibration; query costs retain full standalone cost.','Bootstrap intervals are exploratory and not adjusted for the multiple comparisons here.'],'seconds':round(time.monotonic()-started,3)}
