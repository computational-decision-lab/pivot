...
p=argparse.ArgumentParser();p.add_argument('--new',type=pathlib.Path,required=True);p.add_argument('--repo',type=pathlib.Path,required=True);p.add_argument('--output',type=pathlib.Path,required=True);a=p.parse_args()
reports=[]
for experiment,filename in [('e3c','closed_loop_summary.json'),('e5c','efficiency_summary.json'),('e7c','strategic_summary.json')]:
    current=a.new/experiment/filename;reference=a.repo/'results/v9'/(experiment+'-confirmatory')/filename
