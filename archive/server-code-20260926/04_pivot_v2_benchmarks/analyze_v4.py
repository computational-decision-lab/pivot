...
def set_candidate_geometry(protocol):
    global GRID, DISTANCES, CAPS, PRIMARY_CAP, QUERY_COST
...
def fit_spec(train: list[dict], h: int):
    ...
    # PIVOT_R_FLOOR: frozen protocols use the default 0.25 (Melting Pot return scale). Only set this env var for
    # clearly-labelled sensitivity analyses (e.g. poker units, where 0.25 is ~5-10x the measured instrument noise).
    r_floor = float(os.environ.get("PIVOT_R_FLOOR", "0.25"))
    return fit_posterior_v2(g, ab, sab, distances=DISTANCES if DISTANCES is not None else np.abs(GRID - .5),
                            r_floor=r_floor)
...
def author_features(k: int) -> np.ndarray:
    ...
    x = (GRID - 0.5) / 0.25   # Melting Pot: p in [.125,.875]; OpenSpiel: alpha in [0,1] (alpha=0 is the incumbent copy)
