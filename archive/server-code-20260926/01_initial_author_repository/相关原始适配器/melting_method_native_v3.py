...
def execute(a,backend):
    out=a.output;out.mkdir(parents=True,exist_ok=False);protocol=v2.read(a.protocol)
    ...
    v2.write(out/'protocol.json',{'version':VERSION,'seed':a.seed,'config':c,'batch_protocol_sha256':v2.sha(a.protocol),'backend':backend.metadata})
    v2.write(out/'candidates.json',{'probabilities':GRID,'incumbent':.5,'operator':'fixed symmetric mixture edits, no outcome-dependent proposal selection'})
    ...
    summary={'status':'complete','version':VERSION,'seed':a.seed,'actual_native_episodes':len(episodes.costs),'actual_native_frames':episodes.frames,
...
def main():
    ...
    p.add_argument('--calibration',type=Path,required=True);p.add_argument('--model-path',type=Path,required=True);p.add_argument('--crossbench-root',type=Path,required=True);p.add_argument('--max-seconds',type=int,default=14400)
    a=p.parse_args()
