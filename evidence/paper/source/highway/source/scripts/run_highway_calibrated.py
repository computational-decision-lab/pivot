...
# A separate seed namespace is used for the preregistered replication.  These
# seeds are disjoint from both the original calibrated run and each other.
REPLICATION_CALIBRATION_SEEDS = tuple(2300003 + 37 * index for index in range(20))
REPLICATION_TEST_SEEDS = tuple(2400003 + 37 * index for index in range(60))
REPLICATION_PROTOCOL_ID = "highway-physical-reactive-calibrated-replication-v1"
REPLICATION_CALIBRATION_NAMESPACE = "highway-calibration-20260921-replication-v1"
REPLICATION_TEST_NAMESPACE = "highway-test-20260921-replication-v1"
REPLICATION_BUDGETS = (2, 4)
...
def _seed_rng(seed: int, method: str, protocol_id: str) -> np.random.Generator:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    protocol_id = REPLICATION_PROTOCOL_ID if args.replication else PROTOCOL_ID
    calibration_namespace = (
        REPLICATION_CALIBRATION_NAMESPACE if args.replication else CALIBRATION_NAMESPACE
    )
    test_namespace = REPLICATION_TEST_NAMESPACE if args.replication else TEST_NAMESPACE
    registered_calibration_seeds = (
        REPLICATION_CALIBRATION_SEEDS if args.replication else CALIBRATION_SEEDS
    )
    registered_test_seeds = REPLICATION_TEST_SEEDS if args.replication else TEST_SEEDS
    registered_budgets = REPLICATION_BUDGETS if args.replication else BUDGETS
    config = {
        "protocol_id": protocol_id,
        "calibration_namespace": calibration_namespace,
        "test_namespace": test_namespace,
        "calibration_seeds": list(registered_calibration_seeds if not args.smoke else registered_calibration_seeds[:2]),
        "test_seeds": list(registered_test_seeds if not args.smoke else registered_test_seeds[:2]),
        ...
        "claim_scope": (
            "external_DEV_replication_not_paper_primary_claim"
            if args.replication
            else "external_DEV_redesign_not_paper_primary_claim"
        ),
        "power_note": "60 held-out roots; B=2 is primary and B=4 is secondary",
        "analysis_plan": (
            "paired Uniform HF minus Calibrated PIVOT-KG ISR; percentile bootstrap CI95; "
            "no seed or outcome exclusion; calibration sealed before test"
        ),
    }
    ...
    def evaluate_unit(
        *,
        ...
        key = _unit_key(cohort, seed, phase, method, budget, policy_id, protocol_id)
    ...
    try:
        ...
        for seed in test_seeds:
            ...
            for method in METHODS:
                method_budgets = (ALL_HF_BUDGET,) if method == "All-HF Oracle" else registered_budgets
                for budget in method_budgets:
                    posterior = GaussianPosterior(rows, model) if method == "Calibrated PIVOT-KG" else None
                    rng = _seed_rng(seed, method, protocol_id)
    ...
    summary = {
        "protocol_id": protocol_id,
        "status": (
            "DEV_REPLICATION_SMOKE"
            if args.smoke and args.replication
            else "DEV_SMOKE"
            if args.smoke
            else "DEV_EXTERNAL_REPLICATION"
            if args.replication
            else "DEV_EXTERNAL_REDESIGN"
        ),
        ...
        "budget_contrasts": [_contrast(promotion_rows, budget=budget) for budget in registered_budgets],
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--replication",
        action="store_true",
        help="run the preregistered fresh-seed replication (B=2 primary, B=4 secondary)",
    )
